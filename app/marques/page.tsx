...
export default function MarquesGaleriePage() {
    ...
    (async () => {
      ...
      if (user) {
        const { data: showroom } = await supabase
          ...
          .limit(1)
          .maybeSingle();
        if (showroom) setActiveShowroom(showroom as Showroom);
      }

      const { data: productsData } = await supabase.from('products').select('brand_id').not('brand_id', 'is', null);
      ...
      for (const bb of (brandBadgesData as { brand_id: number; badge_id: number }[]) ?? []) {
  ...
  async function handleContact(brandId: number) {
    if (!activeShowroom) return;
    setContactingId(brandId);
    try {
      ...
      const { data: created } = await supabase
        .from('conversations')
        .insert({ brand_id: brandId, showroom_id: activeShowroom.id })
        .select('id')
        .single();
  ...
  const filteredBrands = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter((b) => b.brand_name?.toLowerCase().includes(q) || b.description?.toLowerCase().includes(q));
    }
        ...
        <div className="max-w-6xl mx-auto px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            >
              ...
              {selectedBadgeIds.size > 0 && <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-white/20 text-xs font-bold">{selectedBadgeIds.size}</span>}
            </button>
          </div>
          ...
          {hasActiveFilters && (
            <div className="mt-2 flex items-center gap-2 text-xs text-neutral-500">
              <span>{filteredBrands.length} marque{filteredBrands.length !== 1 ? 's' : ''} trouvée{filteredBrands.length !== 1 ? 's' : ''}</span>
              ...
              >
                {activeShowroom ? (
                  <div className="flex gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() => handleContact(brand.id)}
                      disabled={contactingId === brand.id}
                    ...
                    >
                      ...
                      {contactingId === brand.id ? 'Ouverture…' : 'Contacter'}
                    </button>
                  </div>
                ) : (
                  <Link
