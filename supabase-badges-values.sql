...
COMMENT ON TABLE public.badges IS 'Badges de valeurs (Made in France, Bio, etc.) affichés sur les fiches marques et boutiques.';

-- 2) Jointure marques <-> badges (max 5 par marque côté UI)
...
-- 3) Jointure showrooms (boutiques) <-> badges (max 5 par boutique côté UI)
...
-- RLS brand_badges : lecture publique, écriture propriétaire
...
-- RLS showroom_badges : lecture publique, écriture propriétaire
...
INSERT INTO public.badges (slug, label, icon, sort_order) VALUES
  ('made-in-france', 'Made in France', '🇫🇷', 1),
  ('bio', 'Bio', '🌿', 2),
  ('upcycling', 'Upcycling', '♻️', 3),
  ('artisanal', 'Artisanal', '🖐️', 4),
  ('vegan', 'Vegan', '🌱', 5),
  ('zero-dechet', 'Zéro Déchet', '🗑️', 6),
  ('made-in-europe', 'Made in Europe', '🇪🇺', 7),
  ('kids-friendly', 'Kids Friendly', '👶', 8),
  ('bien-etre', 'Bien-être', '🧘', 9)
