...
export default function EditProductPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data, error: err } = await supabase.from('products').select('*').eq('id', id).eq('brand_id', brandId).single();
      if (err || !data) {
        setError('Produit introuvable.');
        setLoading(false);
        return;
      }
      const p = data as { product_name: string; price: number; description: string | null; image_url: string | null; commission_percent: number | null; stock_max: number | null };
      setName(p.product_name);
      setPrice(String(p.price));
      setDescription(p.description ?? '');
      setImageUrl(p.image_url ?? '');
      setCommissionPercent(p.commission_percent != null ? String(p.commission_percent) : '');
      setStockMax(p.stock_max != null ? String(p.stock_max) : '');
      setLoading(false);
    })();
  }, [id, brandId]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (brandId == null || Number.isNaN(id)) return;
    setError(null);
    setSaving(true);
    try {
  ...
  if (entityType !== 'brand' || !activeBrand || error) {
    return (
      <div className="max-w-md mx-auto py-8 text-center">
        <p className="text-neutral-600">{error ?? 'Sélectionnez une marque.'}</p>
        <Link href="/admin/products" className="mt-4 inline-block text-sm font-medium text-neutral-900 hover:underline">Retour au catalogue</Link>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      <h1 className="text-xl font-semibold text-neutral-900">Modifier le produit</h1>
      <form onSubmit={handleSubmit} className="mt-6 space-y-4">
        ...
        <div>
          ...
          <input type="text" inputMode="decimal" value={price} onChange={(e) => setPrice(e.target.value)} required className="w-full px-4 py-2.5 rounded-lg border border-neutral-200 bg-white text-neutral-900 focus:outline-none focus:ring-2 focus:ring-neutral-900" />
        </div>
        <div>
        ...
        <div>
          ...
          <input type="text" inputMode="decimal" value={commissionPercent} onChange={(e) => setCommissionPercent(e.target.value)} className="w-full px-4 py-2.5 rounded-lg border border-neutral-200 bg-white text-neutral-900 focus:outline-none focus:ring-2 focus:ring-neutral-900" />
        </div>
        <div>
          <label className="block text-sm font-medium text-neutral-700 mb-1">Stock max</label>
          <input type="number" min={0} value={stockMax} onChange={(e) => setStockMax(e.target.value)} className="w-full px-4 py-2.5 rounded-lg border border-neutral-200 bg-white text-neutral-900 focus:outline-none focus:ring-2 focus:ring-neutral-900" />
        </div>
        {error && <p className="text-sm text-red-600">{error}</p>}
        <button type="submit" disabled={saving} className="w-full py-3 rounded-lg bg-neutral-900 text-white font-medium hover:bg-neutral-800 disabled:opacity-60 flex items-center justify-center gap-2">
