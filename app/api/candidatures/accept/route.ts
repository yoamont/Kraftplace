...
/**
 * POST /api/candidatures/accept
 * Décremente credits et reserved_credits de la marque (côté serveur, service_role).
 * Le client doit être le propriétaire de la boutique (showroom) pour accepter.
...
export async function POST(request: NextRequest) {
  ...
  const { error: updateErr } = await admin
    .from('brands')
    .update({
      credits: Math.max(0, c - 1),
      reserved_credits: Math.max(0, r - 1),
    })
    .eq('id', brandId);
  if (updateErr) {
