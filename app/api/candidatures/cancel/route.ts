...
/**
 * POST /api/candidatures/cancel
 * Décremente reserved_credits de la marque (côté serveur, service_role).
 * Le client doit être le propriétaire de la marque pour annuler sa candidature.
 * Body: { conversationId }
 */
export async function POST(request: NextRequest) {
  ...
  const { data: brand } = await supabase
    .from('brands')
    .select('id')
    .eq('id', (conv as { brand_id: number }).brand_id)
    .eq('owner_id', user.id)
    .single();
  if (!brand) {
    return NextResponse.json({ error: 'Non autorisé : vous devez être le propriétaire de la marque' }, { status: 403 });
  }
